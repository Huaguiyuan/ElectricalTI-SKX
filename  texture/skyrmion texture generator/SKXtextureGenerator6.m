...
%file = '/home/tbhowmick/NetBeansProjects/2X2 TI SKX/NOV 3/BerryCurvatureJbtTvar/INPUT/input.txt';
file = '/home/tbhowmick/NetBeansProjects/2X2 TI SKX/ELECTRICAL/mar 14 2017/BerryCurvatureVer2/INPUT/input.txt';
%file = 'input.txt';
fid = fopen(file,'w'); %where Skyrmion texture would be stored
%columns in the input file are as follows: x y z sx sy sz JH t Phi
...
N = 5;  %total number of grid points in a square lattice, here N= 5 means 
        %5 by 5 square grid
N2 = 5;  % Skyrmion diameter in #grid point       
...
lambda2 = ((N2-1)/2); %Skyrmion radius (in terms of grid points); assuming N would always be odd, skyrmion
                      %center would always be at zero/center
...
%title('Skyrmion texture 11 by 11 grid');
xlabel('X(A^{0})');
ylabel('Y(A^{0})');
% axis off;
