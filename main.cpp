...
int main (int argc, char** argv )
{
    ...
    Haha.GenerateNeighborList(1.00001, 1.00001, true, false, false);
    ...
    /*
    ElectronSystem World("input.txt","openBoundaries.txt", -1.0);
    ofstream outputHamiltonian("Hamiltonian.txt");
    World.Hamiltonian.print(outputHamiltonian);
    World.PrintBoundaryList();
    ...
    for (double energy = -7.0; energy <= 7.0; energy += 14.0/100.)
    {
        printf("Energy = %lf\n", energy);
        cx_mat T = World.ThermalAverageTransmission(0.0, energy);
        fprintf(fp, "%lf\t%lf\n", energy, real(T(0,1)));
    }
    fclose(fp);
    cx_mat T = World.ThermalAverageTransmission(0.0, 0.0);
    */
   // CalculateInjectionBandStructure(-0.2);
    /*double Miu2, Miu4, Miu1, I, Sx, Sy, Sz;
    Miu1 = 0.1;
    FILE* fp;
    fp = fopen("CurrentEfSweep.txt","w");
    double Up, Down;
    double Ef=-6.5;
    CalculateHallEffect(0.0, Ef, Miu1, Miu2, Miu4, I, Sz, Up, Down);*/
    //printf("%lf\t% le\t% le\t% le\t% le\t% le\t% le\n", Ef, I, (Miu2-Miu4)/2.0/Miu1, -Sz, -Sz/I, Up, Down);
    /*for (double Ef = -6.0; Ef <=6.0; Ef+= 12.0/100)
    {
        CalculateHallEffect(0.0, Ef, Miu1, Miu2, Miu4, I, Sz, Up, Down);
        if (fabs(I) < 1.0)
            Sz = 0.0;
        ...
        fprintf(fp, "%lf\t% le\t% le\t% le\t% le\t% le\t% le\n", Ef, I, (Miu2-Miu4)/2.0/Miu1, -Sz, -Sz/I, Up, Down);
   ...
   // cx_mat T = World.Transmission(0.000000);
    //T.print("T=");
    //T.print();
    //printf("test successful.\n");
    //World.GR.print();
    //cx_mat T = World.Transmission(0.0);
    //T.print();
    //World.ListOfSites[0].Location.print();
}
